...
`javac` compiler. The whole stack (Moleculer-Java 2.1.1, Spring Boot 3.5, Jakarta EE 10) requires
Java 17+.
...
- the **64-bit Windows Installer** (`mvn -Pinstaller package` → `installer/dist/moleculer_setup_2.1.1.exe`)
...
4. compiles `installer/moleculer.config.iss` into `installer/dist/moleculer_setup_2.1.1.exe`
...
The executable installer is generated into the `installer/dist` directory, as `moleculer_setup_2.1.1.exe`.
