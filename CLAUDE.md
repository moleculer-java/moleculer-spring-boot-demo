...
This is a **Maven** project (one `pom.xml`) targeting **Java 21** with `javac` (`<maven.compiler.release>21</maven.compiler.release>`). It runs on **Spring Boot 3.5** (jakarta) and the `2.0.0` Moleculer/datatree stack. Version `2.0.0` (Maven coordinate `2.0.0-SNAPSHOT` while the workspace is in active development); **not** published to Maven Central (it's a demo).
