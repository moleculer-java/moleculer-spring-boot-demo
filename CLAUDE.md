...
This is a **Maven** project (one `pom.xml`) targeting **Java 17** with `javac` (`<maven.compiler.release>17</maven.compiler.release>`). Build JDK 17+ (JDK 25 in use); minimum runtime: **JDK 17** (Spring Boot 3.5 requires Java 17+). It runs on **Spring Boot 3.5** (jakarta) and the `2.2.0` Moleculer/datatree stack. Version `2.2.0`; **not** published to Maven Central (it's a demo).
...
mvn -Pinstaller package       # ALSO build the Windows Installer (Windows-only, opt-in):
                              ...
                              #   ISCC.exe                -> installer/dist/moleculer_setup_2.2.0.exe
```
