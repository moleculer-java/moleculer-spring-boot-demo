...
[Files]
; bin = service scripts + Commons Daemon procrun (prunsrv.exe / prunmgr.exe)
Source: installer\bin\*;  DestDir: {app}\bin;  Flags: recursesubdirs createallsubdirs replacesameversion
; jre = minimal JDK 21+ runtime generated by jlink (mvn -Pinstaller package -> target\jre)
Source: target\jre\*;     DestDir: {app}\jre;  Flags: recursesubdirs createallsubdirs replacesameversion
; lib = runtime dependency JARs + the app JAR (maven-dependency-plugin + maven-jar-plugin -> target\lib)
Source: target\lib\*.jar; DestDir: {app}\lib;  Flags: recursesubdirs createallsubdirs replacesameversion
...
[UninstallRun]
Filename: {app}\bin\production-stop.bat; WorkingDir: {app}\bin; Flags: skipifdoesntexist; StatusMsg: Stopping service...
Filename: {app}\bin\prunsrv.exe; Parameters: "//DS//MoleculerJava"; WorkingDir: {app}\bin; Flags: skipifdoesntexist; StatusMsg: Uninstalling service...
...
Root: HKLM32; Subkey: SOFTWARE\Apache Software Foundation\Procrun 2.0\MoleculerJava\Parameters\Java; ValueType: multisz; ValueName: Options; ValueData: "-Djava.net.preferIPv4Stack=true{break}-Djava.io.tmpdir={app}\tmp{break}-Dlogging.config={app}\cfg\logging-production.properties{break}-Dspring.profiles.active=production{break}"
